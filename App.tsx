...
const BildanalyseApp: React.FC<{ onBack: () => void, page: PageEntry, allPages: PageEntry[] }> = ({ onBack, page, allPages }) => {
  ...
  if (isNapoleon) {
    ...
    digitalCheck = data?.digitalCheck || "";
  ...
  const compPage = allPages.find(p => p.id === page.comparisonTarget);
    ...
    <div className="min-h-screen bg-white flex flex-col font-sans text-slate-900 overflow-x-hidden selection:bg-indigo-100">
      <nav className="no-print bg-white/95 backdrop-blur-md border-b border-slate-100 px-6 py-4 flex items-center justify-between sticky top-0 z-50">
        ...
        <div className="flex items-center gap-2 bg-slate-100 p-1.5 rounded-xl">
          {steps.map((step, idx) => (
            <button key={step.number} onClick={() => setActiveStep(idx)} className={`w-10 h-10 rounded-lg flex items-center justify-center text-lg transition-all ${activeStep === idx ? 'bg-white text-slate-900 shadow-sm border border-slate-200' : 'text-slate-400'}`}>
              <span>{step.icon}</span>
            </button>
          ))}
        </div>
      ...
      <div className="no-print bg-[#FFD700] px-6 py-3 flex items-center justify-center text-center z-40">
               ...
               <div className="flex justify-center gap-8">
                  {['red', 'yellow', 'green'].map(color => (
                    <div key={color} className="relative group">
                       ...
                       {color === 'yellow' && (
                         <div className="absolute -top-4 -right-4 bg-indigo-500 text-white text-[8px] font-black px-2 py-1 rounded-full uppercase tracking-widest animate-bounce shadow-xl border border-white/20">Empfohlen</div>
                       )}
                    </div>
                  ))}
               </div>
               {ampelChoice && <div className="mt-12 p-8 bg-slate-900/50 rounded-3xl font-bold text-slate-200 italic border border-white/10 animate-in fade-in zoom-in-95 duration-300">{(ampelFeedback as any)[ampelChoice]}</div>}
            </div>
          </div>
        )}

        <div className="flex items-center justify-between pt-10 pb-20">
          <button disabled={activeStep === 0} onClick={() => { setActiveStep(prev => prev - 1); window.scrollTo({top: 0, behavior: 'smooth'}); }} className="flex items-center gap-4 py-5 px-10 rounded-[1.5rem] font-black uppercase text-xs text-slate-400 border-2 border-slate-200 bg-white hover:bg-slate-50 transition-all disabled:opacity-0">Zurück</button>
          <button onClick={() => { if (activeStep < 4) { setActiveStep(prev => prev + 1); window.scrollTo({top: 0, behavior: 'smooth'}); } else { handlePrint(); } }} className="flex items-center gap-4 py-6 px-14 rounded-[1.5rem] font-black uppercase text-xs text-white bg-indigo-600 shadow-2xl border-b-8 border-indigo-900 hover:bg-indigo-700 transition-all">
...
const App: React.FC = () => {
  ...
  const displayedPages = useMemo(() => {
    ...
    if (difficultyFilter) pages = pages.filter(p => p.difficulty === difficultyFilter);
    return pages;
  }, [config, searchQuery, difficultyFilter]);
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white gap-4">
      ...
      <p className="font-black uppercase text-xs text-slate-400 tracking-widest">Lade Daten...</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white selection:bg-slate-900 selection:text-white font-sans overflow-x-hidden">
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto p-6 md:p-12 no-print">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16 transition-all duration-500 min-h-[400px]">
          {displayedPages.map((page) => (
            <div key={page.id} onClick={() => navigateTo(page.path)} className="group cursor-pointer flex flex-col space-y-6 animate-in fade-in zoom-in-95 duration-700">
              <div className="relative aspect-[4/5] overflow-hidden bg-slate-50 border-2 border-slate-100 rounded-2xl p-3 bg-white shadow-md transition-all group-hover:shadow-2xl group-hover:border-indigo-100 group-hover:-translate-y-2">
                ...
                <div className="absolute bottom-6 left-6 text-white font-black text-xl italic uppercase tracking-tighter opacity-0 group-hover:opacity-100 transition-opacity">Studie beginnen</div>
              </div>
              <div className="text-left px-2">
                <div className="flex gap-2 mb-3">
                  <span className="text-[9px] font-black text-amber-500 bg-amber-50 uppercase tracking-widest border border-amber-100 px-2 py-0.5 rounded-md">{'★'.repeat(page.difficulty)}</span>
